	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			if(currPage==1){
				...
				sb.append("	<li><a href=\"#@\" onclick=\"nextPage("+(currPage-1)+")\">上页</a></li>\n");
			}
			...
			if(currPage==totalPage){
				...
				sb.append("	<li><a href=\"#@\" onclick=\"nextPage("+(currPage+1)+")\">下页</a></li>\n");
				sb.append("	<li><a href=\"#@\" onclick=\"nextPage("+totalPage+")\">尾页</a></li>\n");
			}
			sb.append("	<li class=\"pageinfo\">第"+currPage+"页</li>\n");
			...
			sb.append("		if(url.indexOf('?')>-1){url += \"&"+(entityOrField?"currPage":"page.currPage")+"=\";}\n");
			sb.append("		else{url += \"?"+(entityOrField?"currPage":"page.currPage")+"=\";}\n");
			sb.append("		document.forms[0].action = url+page;\n");
			sb.append("		document.forms[0].submit();\n");
			sb.append("	}else{\n");
			sb.append("		var url = document.location+'';\n");
			sb.append("		if(url.indexOf('?')>-1){\n");
			sb.append("			if(url.indexOf('currPage')>-1){\n");
			sb.append("				var reg = /currPage=\\d*/g;\n");
			sb.append("				url = url.replace(reg,'currPage=');\n");
			sb.append("			}else{\n");
			sb.append("				url += \"&"+(entityOrField?"currPage":"page.currPage")+"=\";\n");
			sb.append("			}\n");
			sb.append("		}else{url += \"?"+(entityOrField?"currPage":"page.currPage")+"=\";}\n");
